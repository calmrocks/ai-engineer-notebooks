...
| ✅ Do | ❌ Anti-pattern |
|---|---|
| Remember RAG = *retrieve then generate*; the retrieval step can be anything | Conflate "RAG" with "embeddings/vector DB"; keyword/SQL/API count too |
...
| Default to **LoRA** over full fine-tuning: cheap, single-GPU, hot-swap the adapters, less forgetting | Full-fine-tune by default; reserve it for drastic change (new language, deep reasoning) |
...
| Defense in depth, so no single failure is catastrophic | Believe any single "prompt-injection fix" makes you safe |
